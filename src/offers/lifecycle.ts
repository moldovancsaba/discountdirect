...
const transitions: Record<OfferStatus, Partial<Record<OfferStatus, OfferTransition["actor"] | OfferTransition["actor"][]>>> = {
  draft: { pending: "seller", withdrawn: ["seller", "system"] },
  pending: { accepted: "buyer", declined: "buyer", expired: "system", cancelled: "seller", sold_out: "system", withdrawn: ["seller", "system"] },
  accepted: { redeemed: "system", expired: "system", cancelled: "seller", sold_out: "system", withdrawn: ["seller", "system"] },
...
export function offerTransition(from: unknown, to: unknown, actor: unknown, reasonCode: unknown): OfferTransition {
  ...
  if (!(allowed === actor || (Array.isArray(allowed) && allowed.includes(actor as OfferTransition["actor"])))) throw new Error("ILLEGAL_OFFER_TRANSITION");
