...
import { recordCampaignAcceptance } from "@/lib/redis";
...
function output(row: any) {
  return {
    ...
    reason: {
      code: row.reasonCode,
      text: row.reasonText,
      evidencePurchaseIds: row.evidencePurchaseIds.map((id: any) =>
        id.toString(),
      ),
    },
    ...
    referencePriceHuf: row.referencePriceHuf ?? row.originalHuf,
    discountBaseHuf: row.discountBaseHuf ?? row.originalHuf,
    referencePriceWindowStart: row.referencePriceWindowStart ?? null,
    referencePriceCalculatedAt: row.referencePriceCalculatedAt ?? null,
...
async function sellerContext(userId: string, sellerSlug: string) {
  ...
  if (!seller) throw new OfferError("NOT_FOUND");
  if (
    ...
    throw new OfferError("FORBIDDEN");
  return seller;
}
function createInput(input: unknown) {
  if (!input || typeof input !== "object") throw new OfferError("INVALID");
  const value = input as Record<string, unknown>;
  if (
    !mongoose.isValidObjectId(value.previewId) ||
    !mongoose.isValidObjectId(value.productId) ||
    !Number.isInteger(value.discountPct) ||
    Number(value.discountPct) < 0 ||
    Number(value.discountPct) > 100 ||
    typeof value.clientRequestId !== "string" ||
    !/^[A-Za-z0-9_-]{8,120}$/.test(value.clientRequestId) ||
    typeof value.expiresAt !== "string"
  )
    throw new OfferError("INVALID");
  const expiresAt = new Date(value.expiresAt);
  if (
    Number.isNaN(expiresAt.getTime()) ||
    expiresAt <= new Date() ||
    expiresAt.getTime() > Date.now() + MAX_EXPIRY_MS
  )
    throw new OfferError("INVALID");
  return {
    previewId: String(value.previewId),
    productId: String(value.productId),
    discountPct: Number(value.discountPct),
    clientRequestId: value.clientRequestId,
    expiresAt,
  };
}

function offerThreadPreview(
  row: any,
  eventType: "created" | "accepted" | "declined" | "expired" | "cancelled",
) {
  ...
  if (eventType === "created")
    return `Ajánlat: ${row.productName}, ${row.discountPct}% kedvezmény, ${price}`;
...
export async function createOffer(
  ...
  return withSellerTenant(seller._id, async () => {
    ...
    const preview = await RecommendationPreview.findOne({
      _id: value.previewId,
      sellerId: seller._id,
      status: "eligible",
    }).lean();
    ...
    const recommendation = preview.recommendations.find(
      (item: any) => item.productId.toString() === value.productId,
    );
    const sendDecision = await maySendMarketing(
      seller._id.toString(),
      preview.buyerUserId.toString(),
      preview.channel,
    );
    const pricingDecision = await discountDecision(
      seller._id.toString(),
      preview.buyerUserId.toString(),
      value.discountPct,
    );
    if (
      ...
      !(await BuyerRelationship.exists({
        sellerId: seller._id,
        buyerUserId: preview.buyerUserId,
        status: "active",
      })) ||
    ...
    let reference;
    try {
      reference = await productReferencePrice(
        seller._id,
        recommendation.productId,
        calculatedAt,
      );
    } catch {
      throw new OfferError("CONFLICT");
    }
    const { product, evidence } = reference;
    if (product.version !== recommendation.productVersion)
      throw new OfferError("CONFLICT");
    const calculatedPrice = discountedPrice(
      product.priceHuf,
      evidence,
      value.discountPct,
    );
    ...
    try {
      await database.connection.transaction(async (session) => {
          ...
          [
            {
              ...
              recommendationPreviewId: preview._id,
              channel: preview.channel,
              productId: recommendation.productId,
              productSku: recommendation.productSku,
              productName: recommendation.productName,
              productVersion: recommendation.productVersion,
              reasonCode: recommendation.reasonCode,
              reasonText: recommendation.reasonText,
              evidencePurchaseIds: recommendation.evidencePurchaseIds,
              originalHuf: product.priceHuf,
              referencePriceHuf: evidence.referencePriceHuf,
              discountBaseHuf: calculatedPrice.discountBaseHuf,
              referencePriceWindowStart: evidence.windowStart,
              referencePriceCalculatedAt: evidence.calculatedAt,
              referencePriceEvidenceVersions: evidence.evidenceVersions,
              discountPct: value.discountPct,
              priceHuf: calculatedPrice.priceHuf,
        ...
        const deliverySnapshot = {
          ...
          priceHuf: row.priceHuf,
          discountPct: row.discountPct,
          referencePriceHuf: row.referencePriceHuf,
          expiresAt: row.expiresAt,
        };
        await createDeliveryRecord(session, {
          sellerId: seller._id,
          buyerUserId: preview.buyerUserId,
          customerId: preview.customerId,
          offerId: row._id,
          kind: "personal_offer",
          channel: "in_app",
          idempotencyKey: `offer:${row._id}:in_app`,
          contentSnapshot: deliverySnapshot,
          createdByUserId: userId,
        });
        await createDeliveryRecord(session, {
          sellerId: seller._id,
          ...
          kind: "personal_offer",
          channel: preview.channel,
          idempotencyKey: `offer:${row._id}:${preview.channel}`,
          contentSnapshot: deliverySnapshot,
          createdByUserId: userId,
        });
        if (conversation) {
          ...
          const updated = await Conversation.findOneAndUpdate(
            { _id: conversation._id, sellerId: seller._id },
            {
              $set: {
                lastEventAt: now,
                lastEventPreview: previewText.slice(0, 240),
              },
      ...
      if (error?.code === 11000) {
        const replay = await Offer.findOne({
          sellerId: seller._id,
          createdByUserId: userId,
          clientRequestId: value.clientRequestId,
        }).lean();
...
export async function buyerOffers(userId: string) {
  await connectDatabase();
  const sellerIds = await activeBuyerSellerIds(userId);
  if (!sellerIds.length) return { offers: [] };
  const now = new Date();
  const rows = await withTenantBypass("buyer-aggregate-offers", async () => {
    await Offer.updateMany(
      ...
      { $set: { status: "expired", decidedAt: now }, $inc: { version: 1 } },
    );
    return Offer.find({ sellerId: { $in: sellerIds }, buyerUserId: userId })
      .sort({ expiresAt: 1, _id: 1 })
      .limit(100)
      .lean();
  });
  return { offers: rows.map(output) };
}

export async function applySystemOfferTransition(
  session: mongoose.ClientSession,
  sellerId: unknown,
  offerId: unknown,
  target: "sold_out" | "withdrawn" | "redeemed",
  reasonCode: string,
  now = new Date(),
) {
...
export async function withdrawOffer(
  ...
  if (!mongoose.isValidObjectId(offerId) || !Number.isInteger(expectedVersion))
    throw new OfferError("INVALID");
  const seller = await sellerContext(userId, sellerSlug);
  const database = await connectDatabase();
  let result: any;
  await database.connection.transaction(async (session) => {
    ...
    const transition = offerTransition(
      row.status,
      "withdrawn",
      "seller",
      "SELLER_WITHDREW_OFFER",
    );
    const now = new Date();
    row.status = transition.to;
    row.decidedAt = now;
...
export async function respondToOffer(
  ...
  if (
    ...
    !["accepted", "declined"].includes(String(decision))
  )
    throw new OfferError("INVALID");
  const database = await connectDatabase();
  const sellerIds = await activeBuyerSellerIds(userId);
  ...
  try {
    await withTenantBypass(
      ...
      async () =>
        await database.connection.transaction(async (session) => {
          ...
          if (!row) throw new OfferError("NOT_FOUND");
          if (
            row.buyerUserId.toString() !== userId ||
            !(await BuyerRelationship.exists({
              sellerId: row.sellerId,
              buyerUserId: userId,
              status: "active",
          ...
          if (row.expiresAt <= now) {
            ...
            if (row.conversationId) {
              ...
              const conversation = await Conversation.findOneAndUpdate(
                { _id: row.conversationId, sellerId: row.sellerId },
                {
                  $set: {
                    lastEventAt: now,
                    lastEventPreview: previewText.slice(0, 240),
          ...
          if (decision === "accepted" && row.campaignId)
            await reserveFlashOffer(session, row, now);
          ...
          if (decision === "accepted")
            await issueCouponForAcceptedOffer(session, row, userId, now);
          if (row.conversationId) {
    ...
    if (error instanceof CampaignError && error.code === "SOLD_OUT")
      throw new OfferError("SOLD_OUT");
    throw error;
  }
}
