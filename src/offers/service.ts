...
function output(row: any) { return { id: row._id.toString(), sellerId: row.sellerId.toString(), product: { id: row.productId.toString(), sku: row.productSku, name: row.productName, version: row.productVersion }, reason: { code: row.reasonCode, text: row.reasonText, evidencePurchaseIds: row.evidencePurchaseIds.map((id: any) => id.toString()) }, campaignId: row.campaignId ? row.campaignId.toString() : null, channel: row.channel, originalHuf: row.originalHuf, referencePriceHuf: row.referencePriceHuf ?? row.originalHuf, discountBaseHuf: row.discountBaseHuf ?? row.originalHuf, referencePriceWindowStart: row.referencePriceWindowStart ?? null, referencePriceCalculatedAt: row.referencePriceCalculatedAt ?? null, discountPct: row.discountPct, priceHuf: row.priceHuf, status: row.status, expiresAt: row.expiresAt, decidedAt: row.decidedAt ?? null, version: row.version, createdAt: row.createdAt }; }
