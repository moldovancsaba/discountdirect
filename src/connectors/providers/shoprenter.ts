...
export const SHOPRENTER_REQUIRED_SCOPES = ["product.product:read", "order.order:read", "store.webhook:read", "store.webhook:write"] as const;
...
function integer(value: unknown, maximum = 1_000_000_000) {
  ...
  if (!Number.isFinite(parsed) || parsed < 0 || parsed > maximum) throw new ProviderError("INVALID_RESPONSE", false);
  const rounded = Math.round(parsed);
  if (Math.abs(parsed - rounded) > 0.0001) throw new ProviderError("INVALID_RESPONSE", false);
  return rounded;
}

function timestamp(value: unknown) {
  return typeof value === "string" && !value.startsWith("0000-") && !Number.isNaN(Date.parse(value)) ? new Date(value) : null;
}
...
function hasNext(body: Json, page: number, count: number) {
  ...
  return Number.isInteger(totalPages) && totalPages > page + 1 ? true : count === 50;
}
...
export class ShoprenterConnector implements CommerceConnector {
  ...
  private async token(signal: AbortSignal) {
    ...
    const response = await providerRequest(this.fetcher, `https://oauth.app.shoprenter.net/${this.configuration.shopName}/app/token`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Accept: "application/json" },
      body: JSON.stringify({ grant_type: "client_credentials", client_id: this.credentials.clientId, client_secret: this.credentials.clientSecret }),
    }, signal);
    ...
    this.accessToken = { value: body.access_token, expiresAt: Date.now() + (Number.isFinite(lifetime) ? Math.max(60, Math.min(lifetime, 86_400)) : 300) * 1_000 };
    return this.accessToken.value;
  }

  private async get(path: string, signal: AbortSignal) {
    const token = await this.token(signal);
    const response = await providerRequest(this.fetcher, `https://${this.configuration.shopName}.api2.myshoprenter.hu/api/${path}`, {
      headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
    }, signal);
    if (!response.ok) throw new ProviderError("INVALID_RESPONSE", false);
    const body = await response.json().catch(() => { throw new ProviderError("INVALID_RESPONSE", false); });
    if (!body || typeof body !== "object" || Array.isArray(body)) throw new ProviderError("INVALID_RESPONSE", false);
    return body as Json;
  }
  ...
  async listProducts(cursor: string | null, signal: AbortSignal): Promise<ConnectorPage<CanonicalProduct>> {
    ...
    const mapped = rows.map((item) => {
      ...
      if (!providerId || !sku) throw new ProviderError("INVALID_RESPONSE", false);
      return { providerId, sku, name: String(item.name ?? item.productName ?? sku).trim(), priceHuf: integer(item.price), active: String(item.status) === "1" && String(item.orderable) === "1", updatedAt: timestamp(item.dateUpdated) };
    });
    return { items: mapped, nextCursor: hasNext(body, page, rows.length) ? String(page + 1) : null };
  }
  ...
  async listOrders(cursor: string | null, signal: AbortSignal): Promise<ConnectorPage<CanonicalOrder>> {
    ...
    const mapped = rows.map((item) => {
      ...
      const currency = String(item.currency ?? item.currencyCode ?? "HUF").toUpperCase();
      if (!providerId || !createdAt || currency !== "HUF") throw new ProviderError("INVALID_RESPONSE", false);
      return { providerId, status: String(item.orderStatus ?? item.status ?? "unknown").slice(0, 80), totalHuf: integer(item.totalGross ?? item.total ?? item.totalPrice ?? 0), createdAt, updatedAt: timestamp(item.dateUpdated) };
    });
    return { items: mapped, nextCursor: hasNext(body, page, rows.length) ? String(page + 1) : null };
  }
  ...
  async getStock(skus: string[], signal: AbortSignal): Promise<CanonicalStock[]> {
    ...
    return items(body).filter((item) => wanted.has(String(item.sku ?? ""))).map((item) => ({ providerId: String(item.id ?? item.innerId ?? ""), sku: String(item.sku), quantity: integer(item.quantity ?? item.stock1 ?? 0, 1_000_000), updatedAt: timestamp(item.dateUpdated) }));
  }
