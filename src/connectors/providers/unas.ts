...
const list = (value: unknown): Xml[] =>
  Array.isArray(value)
    ? (value as Xml[])
    : value && typeof value === "object"
      ? [value as Xml]
      : [];
const finite = (value: unknown, maximum = 1_000_000_000) => {
...
export class UnasConnector implements CommerceConnector {
  ...
  constructor(
    ...
    if (typeof credentials.apiKey !== "string" || credentials.apiKey.length < 8)
  ...
  private async post(
    ...
    const response = await providerRequest(
      ...
      { method: "POST", headers, body: builder.build({ Params: params }) },
      signal,
    );
  ...
  private async token(signal: AbortSignal) {
    ...
    if (typeof login.Token !== "string" || login.Token.length < 8)
      throw new ProviderError("AUTH", false);
    const expiry = date(login.Expire)?.getTime() ?? Date.now() + 300_000;
    this.accessToken = {
      value: login.Token,
      expiresAt: Math.min(expiry, Date.now() + 86_400_000) - 30_000,
    };
    return login.Token;
  }
  ...
  async testConnection(signal: AbortSignal) {
    await this.post(
      "getProduct",
      { LimitNum: 1, LimitStart: 0, ContentType: "minimal" },
      signal,
    );
  }
  ...
  async listProducts(
    ...
    const body = await this.post(
      "getProduct",
      { LimitNum: 100, LimitStart: start, ContentType: "normal" },
      signal,
    );
    ...
    return {
      items: products.map((item) => ({
        ...
        priceHuf: Math.round(finite(item.Price ?? item.PriceNet ?? 0)),
        active:
          String(
            (item.Statuses as Xml | undefined)?.Status ??
              item.StatusBase ??
              "1",
          ) !== "0",
        updatedAt: date(item.LastModTime ?? item.Time),
      })),
      nextCursor: products.length === 100 ? String(start + 100) : null,
    };
  }
  ...
  async listOrders(
    ...
    const body = await this.post(
      "getOrder",
      { LimitNum: 100, LimitStart: start },
      signal,
    );
    const orders = list((body.Orders as Xml | undefined)?.Order);
    return {
      items: orders.map((item) => ({
        providerId: String(item.Key ?? item.Id ?? ""),
        status: String(item.Status ?? item.StatusName ?? "unknown"),
        totalHuf: Math.round(finite(item.SumPriceGross ?? item.Total ?? 0)),
        createdAt: date(item.Date ?? item.Time) ?? new Date(0),
        updatedAt: date(item.Modified ?? item.Time),
      })),
      nextCursor: orders.length === 100 ? String(start + 100) : null,
    };
  }
  ...
  async getStock(
    ...
    const body = await this.post(
      "getStock",
      { Sku: skus.slice(0, 100).join(",") },
      signal,
    );
    return list((body.Products as Xml | undefined)?.Product).map((item) => ({
      providerId: String(item.Id ?? ""),
      sku: String(item.Sku ?? ""),
      quantity: list((item.Stocks as Xml | undefined)?.Stock).reduce(
        (sum, stock) => sum + finite(stock.Qty ?? 0),
        0,
      ),
      updatedAt: null,
    }));
  ...
  async createCheckout(
    ...
    return {
      ...
      providerReference: `unas:${request.handoffId}`,
    };
  }
}
