...
export type OrderWriteBackRequest = {
...
export function connectorProvider(value: unknown): ConnectorProvider | null {
  return typeof value === "string" &&
    CONNECTOR_PROVIDERS.includes(value as ConnectorProvider)
...
export function connectorConfiguration(
  ...
  if (
    typeof source.shopUrl !== "string" ||
    typeof source.checkoutUrlTemplate !== "string"
  )
    return null;
  try {
    const shopUrl = safeCheckoutUrl(source.shopUrl);
    const template = source.checkoutUrlTemplate;
    const probe = safeCheckoutUrl(
      template.replaceAll("{sku}", "SKU").replaceAll("{handoffId}", "handoff"),
    );
    if (
      !template.includes("{sku}") ||
      new URL(shopUrl).origin !== new URL(probe).origin
    )
      return null;
    const shopName =
      typeof source.shopName === "string" &&
      /^[a-z0-9-]{2,80}$/.test(source.shopName)
        ? source.shopName
        : undefined;
