...
import { cancelPostalSubmissionAction,queuePostalSubmissionAction,transitionPostalFulfillmentAction } from "./actions";
import { postalProviderReadiness } from "@/postal/provider";
...
export default async function DeliveriesPage({ params,searchParams }: { params: Promise<{ sellerSlug: string }>;searchParams:Promise<{saved?:string;error?:string}> }) {
  ...
  let data;
  let postal;try { [data,postal] = await Promise.all([listSellerDeliveries(user.id, sellerSlug),listPostalFulfillments(user.id,sellerSlug)]); } catch { redirect("/account?error=forbidden"); }const query=await searchParams;const provider=postalProviderReadiness();
  return <Shell active="account">
    ...
    {query.saved?<BannerNotice severity="success" variant="compact" message={query.saved==="mark_posted"?"A postai szolgáltatónak történő átadás rögzítve. Ez nem kézbesítési igazolás.":query.saved==="provider_queued"?"A szolgáltatói beküldés várólistára került.":query.saved==="provider_cancelled"?"A még el nem fogadott beküldés törölve.":"A nyomtatás rögzítve."}/>:null}{query.error?<BannerNotice severity="error" variant="compact" message="Az állapot nem módosítható. Frissítsd az oldalt, majd ellenőrizd az előző lépést."/>:null}
    {!provider.enabled?<BannerNotice severity="info" variant="compact" message="Automatikus nyomtatási szolgáltató nincs beállítva. A kézi PDF-letöltés és teljesítés továbbra is használható."/>:null}
    <SectionPanel title="Postai teljesítési sor" description="A letöltés, nyomtatás és postai átadás külön auditált lépés. A postai átadás nem jelenti azt, hogy a címzett megkapta a levelet.">
      {!postal.fulfillments.length?<EmptyState title="Nincs nyomtatható postai ajánlat" description="Elfogadott postai ajánlatból, aktív hozzájárulással és címmel hozható létre PDF."/>:<div className="table-wrap"><SimpleDataTable rows={postal.fulfillments.map((item)=>({createdAt:date.format(new Date(item.createdAt)),status:<StatusBadge status={item.status==="posted"?"success":item.status==="printed"?"warning":"info"}>{item.status==="posted"?"Átadva a postai szolgáltatónak":item.status==="printed"?"Kinyomtatva":"Nyomtatásra kész"}</StatusBadge>,provider:item.submission?<StatusBadge status={["delivered","posted","printed","submitted"].includes(item.submission.status)?"success":["failed","retryable_failed"].includes(item.submission.status)?"danger":"info"}>{businessLabel(item.submission.status)}</StatusBadge>:"—",printed:item.printedAt?date.format(new Date(item.printedAt)):"—",posted:item.postedAt?date.format(new Date(item.postedAt)):"—",actions:<div className="inline-actions"><GdsButton component="a" href={`/api/sellers/${sellerSlug}/artifacts/${item.artifactId}`} variant="default" leftSection={<GdsIcon name="Download" decorative/>}>PDF letöltése</GdsButton>{provider.enabled&&!item.submission&&item.status==="ready"?<form action={queuePostalSubmissionAction.bind(null,sellerSlug,item.id)}><GdsButton type="submit" leftSection={<GdsIcon name="Send" decorative/>}>Küldés szolgáltatóval</GdsButton></form>:null}{item.submission&&["queued","retryable_failed"].includes(item.submission.status)?<form action={cancelPostalSubmissionAction.bind(null,sellerSlug,item.submission.id)}><GdsButton type="submit" variant="default" leftSection={<GdsIcon name="Close" decorative/>}>Beküldés törlése</GdsButton></form>:null}{item.status==="ready"&&!item.submission?<form action={transitionPostalFulfillmentAction.bind(null,sellerSlug,"mark_printed",item.id,item.version)}><GdsButton type="submit" leftSection={<GdsIcon name="Print" decorative/>}>Nyomtatás rögzítése</GdsButton></form>:null}{item.status==="printed"?<form action={transitionPostalFulfillmentAction.bind(null,sellerSlug,"mark_posted",item.id,item.version)}><GdsButton type="submit" leftSection={<GdsIcon name="Send" decorative/>}>Postai átadás rögzítése</GdsButton></form>:null}</div>}))} columns={[{key:"createdAt",header:"Létrejött"},{key:"status",header:"Kézi állapot"},{key:"provider",header:"Szolgáltatói állapot"},{key:"printed",header:"Nyomtatva"},{key:"posted",header:"Átadva"},{key:"actions",header:"Műveletek"}]}/></div>}
    </SectionPanel>
