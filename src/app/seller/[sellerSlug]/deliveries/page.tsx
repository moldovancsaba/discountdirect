...
export default async function DeliveriesPage({ params }: { params: Promise<{ sellerSlug: string }> }) {
  ...
  return <Shell active="account">
    <PageHeader title="Kézbesítési napló" description="Tartós csatornaállapotok. A TRANSPORT_NOT_CONFIGURED vagy EMAIL_TRANSPORT_CONFIGURATION_INCOMPLETE állapot azt jelzi, hogy nincs jóváhagyott, teljesen beállított külső szolgáltató." eyebrow={data.seller.name} actions={<GdsButton component="a" href={`/seller/${sellerSlug}/automations`} variant="default">Automatizmusok</GdsButton>} />
