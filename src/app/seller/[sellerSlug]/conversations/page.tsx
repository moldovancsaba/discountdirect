...
export default async function SellerConversationsPage({ params }: { params: Promise<{ sellerSlug: string }> }) {
  ...
  return <Shell active="account">
    <PageHeader title="Beszélgetések" description="Eladóhoz kötött vásárlói beszélgetések élő frissítéssel, ajánlatokkal és vásárlási kontextussal." eyebrow={result.seller.name} actions={<StatusBadge status="info">{result.conversations.length} beszélgetés</StatusBadge>} />
