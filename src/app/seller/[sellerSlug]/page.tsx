...
export default async function SellerPage({ params, searchParams }: { params: Promise<{ sellerSlug: string }>; searchParams: Promise<{ saved?: string; error?: string; batch?: string }> }) {
  ...
  return <Shell active="account">
    <PageHeader title={catalog.seller.name} description="A katalógus minden művelete ehhez az eladóhoz kötött. Az árak egész forintban, a készletek darabban szerepelnek." eyebrow="Eladói munkatér" actions={<StatusBadge status="success" withIcon>{active.length} aktív termék</StatusBadge>} />
