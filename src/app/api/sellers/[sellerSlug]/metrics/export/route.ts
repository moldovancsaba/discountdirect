...
import { metricsCsv } from "@/reporting/export";
...
export async function GET(request: Request, { params }: { params: Promise<{ sellerSlug: string }> }) {
  ...
  if (!result.ready) return errorResponse("REPORT_NOT_READY", "A riport még nem készült el.", 409);
  return new Response(metricsCsv(result.days), { headers: { "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": 'attachment; filename="discountdirect-metrics.csv"', "Cache-Control": "private, no-store" } });
}
