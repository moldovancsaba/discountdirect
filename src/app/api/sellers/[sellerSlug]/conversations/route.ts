...
export async function GET(request: Request, { params }: { params: Promise<{ sellerSlug: string }> }) {
  ...
  try { return Response.json(await sellerConversations(user.id, (await params).sellerSlug, new URL(request.url).searchParams.get("cursor")), { headers: { "Cache-Control": "no-store" } }); }
