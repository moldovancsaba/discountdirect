...
const offerAutomationPreviewSchema = new Schema(
  {
    sellerId: { type: Schema.Types.ObjectId, required: true, ref: "Seller", index: true },
    customerId: { type: Schema.Types.ObjectId, required: true, ref: "Customer", index: true },
    buyerUserId: { type: Schema.Types.ObjectId, required: true, ref: "User", index: true },
    ...
    cadence: { type: String, enum: ["weekly", "fortnightly", "monthly"], required: true },
    nextRunAt: { type: Date, required: true, index: true },
    productLimit: { type: Number, required: true, min: 1, max: 10 },
    status: { type: String, enum: ["ready", "blocked", "scheduled"], required: true, default: "ready", index: true },
    recommendationPreviewId: { type: Schema.Types.ObjectId, default: null, ref: "RecommendationPreview" },
    ...
    createdByUserId: { type: Schema.Types.ObjectId, required: true, ref: "User" },
    scheduledAutomationId: { type: Schema.Types.ObjectId, default: null, ref: "OfferAutomation" },
    scheduledAt: { type: Date, default: null },
  },
  { ...timestamps, collection: "offer_automation_previews" },
);
offerAutomationPreviewSchema.index({ sellerId: 1, createdAt: -1, _id: -1 });
