...
export type AttributionInput = { orderId: string; orderAt: Date; revenueHuf: number; refundedHuf?: number; signedOfferId?: string | null; signedHandoffId?: string | null; campaignOfferId?: string | null; campaignCreatedAt?: Date | null; campaignExpiresAt?: Date | null };
...
export function attributeOrder(input: AttributionInput, cost: ProductCostRevision | null = null, minimumSample = 5): AttributionResult {
  const valid = typeof input.orderId === "string" && input.orderId.length > 0 && boundedMoney(input.revenueHuf) && (input.refundedHuf === undefined || boundedMoney(input.refundedHuf));
  ...
  const marginHuf = warning === "NONE" && cost ? netRevenueHuf - cost.unitCostHuf : null;
