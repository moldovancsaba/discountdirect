...
const deliveryOutboxSchema = new Schema(
  {
    ...
    status: { type: String, enum: ["queued", "processing", "sent", "unsupported", "suppressed", "retryable_failed", "cancelled"], required: true, index: true },
...
const deliveryEventSchema = new Schema(
  {
    ...
    sellerId: { type: Schema.Types.ObjectId, required: true, ref: "Seller", index: true },
    status: { type: String, enum: ["queued", "processing", "sent", "unsupported", "suppressed", "retryable_failed", "cancelled"], required: true },
    reasonCode: { type: String, required: true, maxlength: 120 },
...
export const deliveryModels = [DeliveryOutbox, DeliveryEvent];
