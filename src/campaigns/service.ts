...
import { CustomerMembership } from "@/membership/models";
import { membershipBenefits, type MembershipTier } from "@/membership/core";
...
async function campaignAudience(sellerId: any, productId: string, channel: "email" | "postal", discountPct: number, campaignKey: string) {
  const storedSettings = await SellerSettingsModel.findOne({ sellerId }).lean();
  ...
  const audience: any[] = []; const holdout: any[] = []; const seen = new Set<string>();
  for (const preview of previews) {
    ...
    const membership = await CustomerMembership.findOne({ sellerId, buyerUserId: preview.buyerUserId, status: "active" }).lean();
    const membershipTier = (membership?.tier ?? "member") as MembershipTier;
    seen.add(buyer);
    const snapshot = { buyerUserId: preview.buyerUserId, customerId: preview.customerId, recommendationPreviewId: preview._id, reasonCode: recommendation.reasonCode, reasonText: recommendation.reasonText, evidencePurchaseIds: recommendation.evidencePurchaseIds, membershipTier, freeDelivery: membershipBenefits(membershipTier).freeDelivery };
    if (isHoldout({ sellerId: sellerId.toString(), buyerUserId: buyer, holdoutPct: settings.holdout_pct, mode: settings.holdout_mode, campaignKey })) holdout.push(snapshot);
...
function previewHash(input: ReturnType<typeof previewInputFrom>, product: any, audience: any[], holdout: any[]) {
  return createHash("sha256").update(JSON.stringify({ input: { ...input, expiresAt: input.expiresAt.toISOString() }, product: [product._id.toString(), product.version, product.stock, product.active], audience: audience.map((item) => [item.buyerUserId.toString(), item.recommendationPreviewId.toString(), item.reasonCode]).sort(), holdout: holdout.map((item) => item.buyerUserId.toString()).sort() })).digest("hex");
}
...
export async function createFlashCampaignPreview(userId: string, sellerSlug: string, input: unknown) {
  ...
  const campaignKey = `${value.productId}:${value.channel}:${value.expiresAt.toISOString()}`;
  const allocation = await campaignAudience(seller._id, value.productId, value.channel, value.discountPct, campaignKey); if (!allocation.audience.length) throw new CampaignError("CONFLICT");
...
export async function launchFlashCampaignPreview(userId: string, sellerSlug: string, previewId: string, clientRequestId: unknown) {
  ...
  try { await database.connection.transaction(async (session) => {
    ...
    const [campaign] = await Campaign.create([{ _id: campaignId, sellerId: seller._id, kind: "flash", productId: preview.productId, productSku: preview.productSku, productName: preview.productName, productVersion: preview.productVersion, originalHuf: preview.originalHuf, referencePriceHuf: preview.referencePriceHuf, discountBaseHuf: preview.discountBaseHuf, referencePriceWindowStart: preview.referencePriceWindowStart, referencePriceCalculatedAt: preview.referencePriceCalculatedAt, referencePriceEvidenceVersions: preview.referencePriceEvidenceVersions, discountPct: preview.discountPct, priceHuf: preview.priceHuf, channel: preview.channel, quantity: preview.quantity, remaining: preview.quantity, expiresAt: preview.expiresAt, audienceSnapshot: preview.audienceSnapshot, holdoutSnapshot: preview.holdoutSnapshot, holdoutPct: preview.holdoutPct, holdoutMode: preview.holdoutMode, clientRequestId, createdByUserId: userId }], { session });
    ...
    for (const row of created) {
      const contentSnapshot = { productName: row.productName, priceHuf: row.priceHuf, discountPct: row.discountPct, referencePriceHuf: row.referencePriceHuf, expiresAt: row.expiresAt, campaignId: campaignId.toString(), membershipTier: (preview.audienceSnapshot.find((item: any) => item.buyerUserId.toString() === row.buyerUserId.toString())?.membershipTier ?? "member"), freeDelivery: preview.audienceSnapshot.find((item: any) => item.buyerUserId.toString() === row.buyerUserId.toString())?.freeDelivery === true };
...
async function measurementFor(row: any) {
  const treatmentIds = new Set((row.audienceSnapshot ?? []).map((item: any) => item.customerId.toString()));
  const holdoutIds = new Set((row.holdoutSnapshot ?? []).map((item: any) => item.customerId.toString()));
  const customerIds = [...treatmentIds, ...holdoutIds];
  if (!customerIds.length) return campaignLift({ treatmentSize: treatmentIds.size, treatmentConversions: 0, holdoutSize: holdoutIds.size, holdoutConversions: 0 });
  const purchases = await Purchase.find({ sellerId: row.sellerId, customerId: { $in: customerIds }, status: "purchased", purchasedAt: { $gte: row.createdAt, $lte: row.expiresAt }, $or: [{ productId: row.productId }, { productSku: row.productSku }] }).select({ customerId: 1 }).lean();
  const converted = new Set(purchases.map((purchase: any) => purchase.customerId.toString()));
