...
export async function launchFlashCampaignPreview(userId: string, sellerSlug: string, previewId: string, clientRequestId: unknown) {
  ...
  const seller = await sellerContext(userId, sellerSlug);
  const replay = await Campaign.findOne({ sellerId: seller._id, createdByUserId: userId, clientRequestId }).lean(); if (replay) return campaignOutput(replay, await Offer.countDocuments({ campaignId: replay._id }));
  const preview = await CampaignPreview.findOne({ _id: previewId, sellerId: seller._id }).lean();
  if (!preview) throw new CampaignError("NOT_FOUND");
  if (preview.launchedCampaignId) { const campaign = await Campaign.findById(preview.launchedCampaignId).lean(); if (campaign) return campaignOutput(campaign, await Offer.countDocuments({ campaignId: campaign._id })); }
  if (preview.status !== "ready" || preview.expiresAt <= new Date()) {
    if (preview.status === "ready") await CampaignPreview.updateOne({ _id: preview._id }, { $set: { status: "expired" } });
  ...
  try { await database.connection.transaction(async (session) => {
    ...
    const [campaign] = await Campaign.create([{ _id: campaignId, sellerId: seller._id, kind: "flash", productId: preview.productId, productSku: preview.productSku, productName: preview.productName, productVersion: preview.productVersion, originalHuf: preview.originalHuf, discountPct: preview.discountPct, priceHuf: preview.priceHuf, channel: preview.channel, quantity: preview.quantity, remaining: preview.quantity, expiresAt: preview.expiresAt, audienceSnapshot: preview.audienceSnapshot, clientRequestId, createdByUserId: userId }], { session });
    await CampaignPreview.updateOne({ _id: preview._id, status: "ready" }, { $set: { status: "launched", launchedCampaignId: campaignId, launchedAt: now } }, { session });
    ...
    for (const row of created) {
      const contentSnapshot = { productName: row.productName, priceHuf: row.priceHuf, discountPct: row.discountPct, expiresAt: row.expiresAt, campaignId: campaignId.toString() };
      ...
      if (row.conversationId) {
        ...
        const updated = await Conversation.findByIdAndUpdate(row.conversationId, { $set: { lastEventAt: now, lastEventPreview: previewText.slice(0, 240) }, $inc: { pendingOfferCount: 1, buyerUnreadCount: 1, version: 1 } }, { new: true, session });
  ...
  }); return result; } catch (error: any) { if (error?.code === 11000) { const existing = await Campaign.findOne({ sellerId: seller._id, createdByUserId: userId, clientRequestId }).lean(); if (existing) return campaignOutput(existing, await Offer.countDocuments({ campaignId: existing._id })); } throw error; }
}
