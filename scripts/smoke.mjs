...
if (process.env.OPERATIONS_TOKEN) {
  const response = await fetch(new URL('/api/health/ready', base), { headers: { Authorization: `Bearer ${process.env.OPERATIONS_TOKEN}` } });
...
console.log('HTTP smoke checks passed: home, protected dashboard, liveness, unauthorized readiness and 404.');
