...
- Run `pnpm blob:verify` with the approved Blob variables to produce a synthetic private-artifact drill. It writes one namespaced text object, reads it through the provider and through a five-minute signed URL, verifies the exact content, and deletes the synthetic object in a `finally` cleanup. Record the JSON result with the deployment and timestamp; never use customer keys or content. When Blob is not configured, the command performs no write, emits `{"ok":false,"blocked":true,"reasonCode":"BLOB_NOT_CONFIGURED"}` (or `BLOB_CONFIGURATION_INCOMPLETE`) and exits with code 2; this is an operational blocker, not a passing drill.
